...
/**
 * @param {Disposable[]} items
 */
export function disposeItems(items) {
	for (let i = 0, item; i < items.length; i++) {
		item = items[i];
		if (isBlock(item)) {
			disposeBlock(/** @type {Block} */(item), true);
		} else {
			unmountComponent(/** @type {Component} */(item));
		}
	}
 ...
 */
export function disposeBlock(block, self) {
	disposeItems(block.dispose);
