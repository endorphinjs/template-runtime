 ...
 * @property {Element[] | Block[]} items Current injector contents
...
export function exit(injector) {
	...
	if (stack.length) {
		const lastBlock = last(stack);
		consume(lastBlock, block);
		ins = index(injector, lastBlock) + lastBlock.size + 1;
	} else if (block) {
		reset(block);
		ins = index(injector, block) + block.size + 1;
	}
