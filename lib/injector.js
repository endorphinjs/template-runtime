...
export function insert(injector, node) {
	domInsert(node, injector.parentNode, getAnchorNode(injector.items, injector.ptr));
...
export function move(injector, block, pos) {
	...
	let anchor = getAnchorNode(injector.items, pos);
 ...
 * @returns {Node}
 */
function getAnchorNode(items, ix) {
