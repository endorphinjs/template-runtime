...
function getAnchorNode(items, ix, parent) {
	while (ix < items.length) {
		const item = items[ix++];
		if (getNode(item).parentNode === parent) {
			return item;
		}
	}
}
 ...
 * @param {Node | Component} anchor
 * @returns {Node} Inserted item
 */
function domInsert(node, parent, anchor) {
	return anchor
		? parent.insertBefore(getNode(node), getNode(anchor))
		: parent.appendChild(getNode(node));
}
...
function domRemove(node) {
	const _node = getNode(node);
	const parent = _node.parentNode;
	parent && parent.removeChild(_node);
}

/**
 * Возвращает DOM-узел для указанного элемента дерева
