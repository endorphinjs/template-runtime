...
import { createInjector, disposeItems, disposeBlock } from './injector';
...
export function unmountComponent(elem) {
	...
	disposeItems(componentModel.dispose);

	const { slots } = componentModel;
	for (const slotName in slots) {
		disposeBlock(slots[slotName].block, true);
	}
 ...
 * @param {Disposable} item
 */
export function addDisposable(host, item) {
	if (/** @type {Component} */ (host).componentModel) {
		/** @type {Component} */ (host).componentModel.dispose.push(item);
	} else {
		/** @type {Injector} */ (host).ctx.dispose.push(item);
	}
}
