...
export function createComponent(name, definition, host) {
	...
	element.setProps = (value, silent) => {
		if (isObject(value)) {
			...
			!silent && component.mounted && updateNext(component);
		}
	};

	element.setState = (value, silent) => {
		if (isObject(value) && assignIfNeeded(element.state, value) && !silent && component.mounted) {
			updateNext(component);
		}
	};

	// if (host) {
	// 	host.nested.push(component);
	// }
...
export function mountComponent(component, initialProps) {
	const { element, injector, definition } = component;
...
export function updateComponent(component) {
	const { injector, update } = component;
	finalizeEvents(injector);
 ...
 * @param {Component} component
 */
function updateNext(component) {
	if (!component.rendering) {
		updateComponent(component);
	} else if (!component.queued) {
		component.queued = nextTick(() => updateComponent(component));
	}
}
